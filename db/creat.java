		...
		if(btn.getText().equals("Add"))
			{
				...
				{	
					...
					break label;
						...
						int count=st.executeUpdate("insert into mytable(id,Name,chem,phy,mat,ps,bio) values ("+a+",'"+name+"',"+b+","+c+","+d+","+e+","+f+")");
						con.close();
						st.close();
						}
					catch(Exception ax){
					//ax.printStackTrace();
					JOptionPane.showMessageDialog(null,"Please enter another roll number, this Roll Number is already taken!");
					setVisible(false);
					new creat();
					}
